...
在回测中，处理涨跌停板的关键在于**成交判定**。从市场机制角度看，常见建模原则包括：

1.  **价格检查**：
    *   若 $High = LimitUp$（涨停），通常意味着买盘拥挤，保守回测中往往不应假设还能轻易买入。
    *   若 $Low = LimitDown$（跌停），通常意味着卖盘拥挤，保守回测中往往不应假设还能轻易卖出。

2.  **流动性检查**：
    *   即使价格未封板，如果当前 Bar 的成交量 (Volume) 极小，大额订单也可能无法完全成交。`AKQuant` 当前可通过 `volume_limit` 参数（如 Bar 成交量的 10%）限制最大成交量。

3.  **特殊状态**：
    *   **一字板**：如果 $Open = High = Low = Close = LimitUp/LimitDown$，且 $Volume \approx 0$，在更保守的建模中通常应视为全天几乎无法成交。
